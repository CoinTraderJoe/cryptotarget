...
*Setup instructions for a gitian build of Cryptotarget using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Cryptotarget
...
- [Building Cryptotarget](#building-cryptotarget)
...
Clone the git repositories for cryptotarget and gitian,

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/cryptotarget/cryptotarget
```
...
In [doc/release-process.md](release-process.md) in the cryptotarget repository under 'Fetch and build inputs'.
...
Building Cryptotarget
----------------

To build Cryptotarget (for Linux and/or Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the cryptotarget repository.

Output from `gbuild` will look something like

    Initialized empty Git repository in /home/debian/gitian-builder/inputs/cryptotarget/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/cryptotarget/cryptotarget
...
For example:
```bash
URL=https://github.com/laanwj/cryptotarget.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit cryptotarget=${COMMIT} --url cryptotarget=${URL} ../cryptotarget/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit cryptotarget=${COMMIT} --url cryptotarget=${URL} ../cryptotarget/contrib/gitian-descriptors/gitian-win.yml
```
...
```bash
    gpg --detach-sign ${VERSION}/${SIGNER}/cryptotarget-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/cryptotarget-build.assert
```
...
[cryptotarget/gitian.sigs](https://github.com/cryptotarget/gitian.sigs/) repository, or
