...
CRYPTOTARGETD=${1}/cryptotargetd
CLI=${1}/cryptotarget-cli
...
B4ARGS="-datadir=$D4"
$CRYPTOTARGETD $CRYPTOTARGETDARGS $B4ARGS &
B4PID=$!
...
function S {
  ...
  if [[ $TXID == "" ]] ; then
      echoerr "node${1}: error sending ${3} crt"
...
function StartThree {
  $CRYPTOTARGETD $CRYPTOTARGETDARGS $B1ARGS &
  B1PID=$!
  $CRYPTOTARGETD $CRYPTOTARGETDARGS $B2ARGS &
  B2PID=$!
  $CRYPTOTARGETD $CRYPTOTARGETDARGS $B3ARGS &
  B3PID=$!
}
